...
const GridItem = ({ area, icon, title, description }: GridItemProps) => {
    return (
        <li className={cn("min-h-[14rem] list-none", area)}>
            <div className="relative h-full rounded-[1.25rem] border border-slate-200/50 p-2 md:rounded-[1.5rem] md:p-3 hover:shadow-xl transition-shadow duration-300">
                <GlowingEffect
                    spread={40}
                    glow={true}
                    disabled={false}
                ...
                <div className="relative flex h-full flex-col justify-between gap-6 overflow-hidden rounded-xl bg-white p-6 md:p-6 glass-card">
                    <div className="relative flex flex-1 flex-col justify-between gap-3">
                        <div className="w-fit rounded-lg border border-amber-200 bg-amber-50 p-2 shadow-sm">
                            {icon}
                        </div>
                        <div className="space-y-3">
                            <h3 className="pt-0.5 text-xl leading-[1.375rem] font-bold font-sans tracking-[-0.02em] md:text-2xl md:leading-[1.875rem] text-balance text-slate-900">
                                {title}
                            </h3>
                            <h2 className="font-sans text-sm leading-[1.3rem] md:text-base md:leading-[1.5rem] text-slate-500 font-light">
                                {description}
                            </h2>
                        </div>
                    </div>
