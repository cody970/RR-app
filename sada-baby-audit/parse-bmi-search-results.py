# Parse BMI Songview search results from browser element data
# Extract all visible titles from the 57 performer groupings
...
# Collect all unique titles from the visible search results
all_titles = set()
for performer, titles in performer_entries.items():
    for t in titles:
        all_titles.add(t.upper().strip())

print(f"Total performer groupings: {len(performer_entries)}")
...
groups_with_more = ["SADA BABY", "BABY SADA", "FMB DZ FEAT SADA BABY", "SORRELL SADA BABY CASADA"]
print("Groups with additional titles (need full catalog):")
for g in groups_with_more:
    print(f"  - {g}")
