# Extract all BMI Songview titles from the browser element data
# This captures the titles visible from the 57 performer groupings on the search page

# Titles from performer groupings with only visible titles (no "+ additional titles")
single_titles = {
...
# Collect all unique titles from single-title groupings
all_titles = set()
for performer, titles in single_titles.items():
    for t in titles:
        all_titles.add(t.upper().strip())

print(f"Titles from {len(single_titles)} performer groupings (no additional): {len(all_titles)} unique")
print()
